...
from Cerrera.control_trace import check_state
...
def check_strig():
    ...
    for item in sstrizhes_dict:
        Record = apps.get_model('geo', 'Strigstate')
        name = item
        ip1_state = check_state(sstrizhes_dict[item][0])
        ip2_state = check_state(sstrizhes_dict[item][1])
        ...
        check_uni = main_check(sstrizhes_dict[item][2])
